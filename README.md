...
Original/early single-cell RNA-seq processing script retained for reference.

The finalized and organized single-cell processing workflow used for the repository is provided in:
...
- loading the processed human esophageal single-cell dataset;
- addition and organization of metadata;
...
- other figure-formatting tasks.

Several figure scripts use:

```r
source("EoE_source.R")
```

or an equivalent local path.
...
The Figure 4 scripts use these results to visualize interactions directed toward basal epithelial cells.
...
For single-cell RNA-seq processing:
...
`EoE_source.R` contains helper functions required by several figure scripts.

`multinichenetr.R` should be run before the MultiNicheNet-dependent portions of `06_Figure4.R`.
...
For complete package and dependency information, it is recommended to add a `sessionInfo.txt` file to the repository root.

It can be generated in R with:
...
- Some scripts contain absolute Windows file paths from the original analysis environment. These paths should be changed to match the user's local directory structure before execution.
- Several figure scripts depend on processed `.RDS` or `.qs` objects generated by upstream analyses.
- Large Seurat objects, raw count matrices, and intermediate analysis files are not intended to be committed to GitHub.
...
Journal and DOI information can be added here after publication.
...
An explicit open-source license can be added before public release if appropriate for the project and institutional requirements.
